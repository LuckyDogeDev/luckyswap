/*
 * This is a specification file for GoldMinerV2's formal verification
 * using the Certora prover.
 *
 * Run this file with scripts/_runGoldMinerV2.sh
 */
...
using DummyGOLN as goldnuggetToken
...
methods {
	...
	poolInfoAccGoldNuggetPerShare(uint256 pid) returns (uint128) envfree
	...
	// GOLN token
	GOLN() returns (address) envfree

	// Rewarder
	onGoldNuggetReward(uint256, address, address, uint256, uint256) => NONDET
...
rule monotonicityOfAccGoldNuggetPerShare(uint256 pid, method f) {
	env e;

	uint128 _poolInfoAccGoldNuggetPerShare = poolInfoAccGoldNuggetPerShare(pid);

	calldataarg args;
	f(e, args);

	uint128 poolInfoAccGoldNuggetPerShare_ = poolInfoAccGoldNuggetPerShare(pid);

	assert compareUint128(poolInfoAccGoldNuggetPerShare_, _poolInfoAccGoldNuggetPerShare);
}
...
rule noChangeToOtherPool(uint256 pid, uint256 otherPid) {
	...
	uint128 _otherAccGoldNuggetPerShare = poolInfoAccGoldNuggetPerShare(otherPid);
	...
	uint128 otherAccGoldNuggetPerShare_ = poolInfoAccGoldNuggetPerShare(otherPid);
	uint64 otherLastRewardBlock_ = poolInfoLastRewardBlock(otherPid);
	uint64 otherAllocPoint_ = poolInfoAllocPoint(otherPid);

	assert(_otherAccGoldNuggetPerShare == otherAccGoldNuggetPerShare_, "accGoldNuggetPerShare changed");
...
rule preserveTotalAssetOfUser(method f, uint256 pid, address user,
	...
	require GOLN() != lpToken(pid); // <-- check this again (Nurit)
...
rule goldnuggetGivenInHarvestEqualsPendingGoldNugget(uint256 pid, address user, address to) {
	env e;

	require to == user && user != currentContract && e.msg.sender == user;
	require goldnuggetToken == GOLN();

	uint256 userGoldNuggetBalance = goldnuggetToken.balanceOf(e, user);
	uint256 userPendingGoldNugget = pendingGoldNugget(e, pid, user);

	// Does success return value matters? Check with Nurit
	harvest(e, pid, to);

	uint256 userGoldNuggetBalance_ = goldnuggetToken.balanceOf(e, user);

	assert(userGoldNuggetBalance_ == (userGoldNuggetBalance + userPendingGoldNugget),
		   "pending goldnugget not equal to the goldnugget given in harvest");
}
...
function callFunctionWithParams(method f, uint256 pid, address sender, address to) {
	...
	} else if (f.selector == pendingGoldNugget(uint256, address).selector) {
		pendingGoldNugget(e, pid, to);
