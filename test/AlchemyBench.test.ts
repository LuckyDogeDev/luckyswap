...
describe("AlchemyBench", function () {
  ...
  beforeEach(async function () {
    ...
    this.bar = await this.AlchemyBench.deploy(this.goldnugget.address)
  ...
  it("should not allow enter if not enough approve", async function () {
    ...
    await this.goldnugget.approve(this.bar.address, "50")
    await expect(this.bar.enter("100")).to.be.revertedWith("ERC20: transfer amount exceeds allowance")
    await this.goldnugget.approve(this.bar.address, "100")
  ...
  it("should work with more than one participant", async function () {
    await this.goldnugget.approve(this.bar.address, "100")
    await this.goldnugget.connect(this.bob).approve(this.bar.address, "100", { from: this.bob.address })
    ...
    expect(await this.goldnugget.balanceOf(this.bar.address)).to.equal("30")
    // AlchemyBench get 20 more GOLNs from an external source.
    await this.goldnugget.connect(this.carol).transfer(this.bar.address, "20", { from: this.carol.address })
    ...
    expect(await this.goldnugget.balanceOf(this.bar.address)).to.equal("52")
