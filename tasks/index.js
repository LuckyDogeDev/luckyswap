...
task("goldminer:add", "Add pool to goldminer")
.setAction(async function (taskArguments, { ethers: { getNamedSigner } }, runSuper) {
  const goldMiner = await ethers.getContract("GoldMiner")

  await (await goldMiner.connect(await getNamedSigner("dev")).add(1000, '0x3e78a806b127c02b54419191571d9379819e989c', true)).wait()
});

task("goldminer:deposit", "GoldMiner deposit")
.addParam("pid", "Pool ID")
.addParam("amount", "Amount")
.setAction(async function ({ pid, amount }, { ethers: { getNamedSigner } }, runSuper) {
  const goldMiner = await ethers.getContract("GoldMiner")

  const { lpToken } = await goldMiner.poolInfo(pid)

  await run("erc20:approve", { token: lpToken, spender: goldMiner.address })

  await (await goldMiner.connect(await getNamedSigner("dev")).deposit(pid, amount)).wait()
});

task("goldminer:withdraw", "GoldMiner withdraw")
.addParam("pid", "Pool ID")
.addParam("amount", "Amount")
.setAction(async function ({ pid, amount }, { ethers: { getNamedSigner } }, runSuper) {
  const goldMiner = await ethers.getContract("GoldMiner")
